...
- garantias
- na intro, falar pq apply to most games, quais são e como e óbivo que se aplicam

- similar metrics in previous work
    - state machines

- inverter related work?

- qual o custo de CPU p/ o pingus?
    - falar do sensys
    - comentar sobre AAA, talvez para partes do jogo?

- brief intro about SSRP, Ceu
...
Ex.: Fig.3.b): how is but.on_click updated? How does the "emit game.armagedon" arrives to pingus?
Using events? (please see below).
Fig.7.b): how is variable game.update modified?

Communication in Ceu requires more detail. Pg. 10 states that events are broadcasted. Broadcast is the core
of the observer pattern and it should be simple to achieve the same effect in OOP. Nevertheless, broadcast communication
can be ineffective and make applications very difficult to understand/debug. The limitations of broadcast should
be discussed in detail. If events are broadcasted, are filters supported?
...
functionality in C++, such as in heavy graphical primitives.
...
\begin{lstlisting}[
  ...
  caption={An example in \CEU that blinks an LED every 500ms in the background
          and awaits a button click or 1 hour to terminate.},
  label={lst.ceu},
]
input  void BUTTON;
output bool LED;
...
%
Computations within a reaction (such as expressions, assignments, and system
calls) are also instantaneous considering the synchronous
...
upcoming events (e.g., mouse clicks and timers).

Listing~\ref{lst.syntax} shows a compact reference of \CEU and
Listing~\ref{lst.ceu} shows a simple example that blinks an LED and terminates
either on a button press or after 1 hour of execution.
%
...
to a coroutine or fiber~\cite{sync_async.cooperative}, but which can also
interact directly with the environment through \code{await} statements
(ln.~7,9).
%
...
In C++, the class \code{Bomber} in Listing~\ref{lst.bomber.a} defines the
...
Listing~\ref{lst.bomber.b} does not rely on state variables and reflects the
sequential state machine implicitly, using \code{await} statements to separate
the effects in direct style.
%
The \code{Bomber} is a \code{code} abstraction of \CEU, which is similar
...
\code{code} abstractions and execute in separate through the \code{spawn}
primitive (ln 4--5).
...
\subsubsection{ Transition from \emph{Story}  to \emph{Credits} and \emph{Worldmap} Screen}
...
From the click in the story dot until the return to the world map, the game
animates the story with a timer-based scrolling and also reacts to user input
to advance it.

In C++, the class \code{StoryDot} in Listing~\ref{lst.story.a} (ln 1--12) first
...
\begin{lstlisting}[
  numbers=left,xleftmargin=0.5cm,
  float=t,
  caption={C++: Transition from \emph{Story} to \emph{Credits} and \emph{Worldmap} screen.},
  label={lst.story.a},
]
...
Continuation passing typically controls the overall structure of games in, such
as screen transitions in menus and level progressions.
%
...
%
However, in \CEU, the \code{Sprite} abstraction can react directly to external
